    ...
    <Container className="mt-5">
      <Card className="p-4">
        ...
        <Row>
          {/* User Panel */}
          <Col md={6}>
            <h4>User Panel (Coinbase Wallet)</h4>
            ...
            <Button onClick={connectUserCoinbaseWallet} className="w-100 mb-3">
              Connect Coinbase Wallet
            </Button>
            {userAccount && <p>Connected Account: {userAccount}</p>}

            <Button onClick={generateKeyPair} className="w-100 mb-3">
              Generate Key Pair
            </Button>
            ...
            <Button
              ...
              disabled={!publicKeyX || !userAccount || !username || loading}
            >
              {loading ? (
            ...
            <Button onClick={getAuditLogs} className="w-100 mb-3">
              Get Audit Logs
            </Button>
            {auditLogs.length > 0 && (
              <ul>
                {auditLogs.map((log, index) => (
                  <li key={index}>
                    User: {log.returnValues.user}, Public Key:{" "}
          ...
          {/* Admin Panel */}
          <Col md={6}>
            <h4>Admin Panel (MetaMask)</h4>
            <Button onClick={connectAdminMetaMask} className="w-100 mb-3">
              Connect MetaMask
            </Button>
            {adminAccount && <p>Admin Account: {adminAccount}</p>}
              ...
              adminRequests.map((request, index) => (
                <Card key={index} className="mb-2">
                  <Card.Body>
                    ...
                    <Card.Text>Public Key: {request.publicKey}</Card.Text>
                    <Card.Text>User Address: {request.userAddress}</Card.Text>
                    {request.approved ? (
